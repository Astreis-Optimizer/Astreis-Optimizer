...
class MainFunctions():
    ...
    # ///////////////////////////////////////////////////////////////
    def set_right_column_menu(self, menu, title="", icon_path=""):
        if menu != None:
            # Update title and icon
            self.ui.right_column.title_label.setText(title)
            if icon_path:
                self.ui.right_column.icon_frame.setStyleSheet(f"background-image: url({icon_path}); background-position: center; background-repeat: no-repeat;")
    ...
    def toggle_right_column(self):
        # GET WIDTH
        width = self.ui.right_column_frame.width()
        right_column_width = self.ui.right_column_frame.maximumWidth()

        # SET MAX WIDTH
        if width == 0:
            width_extended = right_column_width
        else:
            width_extended = 0

        # ANIMATION
        self.animation = QPropertyAnimation(self.ui.right_column_frame, b"minimumWidth")
